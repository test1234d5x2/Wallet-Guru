...
export default async function getGoals(token: string): Promise<Goal[]> {
    ...
    const goals: Goal[] = data.goals.map((goal: any) => new Goal(goal.title, goal.userID, goal.description, goal.target, goal.targetDate, goal.status, goal.id, goal.current));
    return goals;
}